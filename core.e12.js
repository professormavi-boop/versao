...
function buildNav(){let nav=S.profile.role==='student'?[...NAV_STUDENT]:[...NAV_TEACHER];if(S.profile.role==='super_admin')nav=[['management','Gestão da plataforma']];if(BETA_PROPOSALS&&S.profile.role==='teacher')nav=[['home','Início'],['teacher-organization','Minhas instituições'],['proposals','Propostas'],['live','Ao Vivo'],['correction','Correção'],['teacher-account','Conta e créditos']];$('nav').innerHTML=nav.map(([r,l])=>`<button data-route="${r}"><span class="dot"></span>${l}</button>`).join('');if(S.profile.role==='super_admin')$('nav').innerHTML=ADMIN_NAV.map(([label,items])=>`<details class="admin-nav-group" ${items.some(([r])=>r===S.route)||label==='Visão geral'?'open':''}><summary>${esc(label)}</summary>${items.map(([r,l])=>`<button data-route="${r}">${esc(l)}</button>`).join('')}</details>`).join('');$('nav').onclick=e=>{const b=e.target.closest('[data-route]');if(b){navigate(b.dataset.route);closeDrawer()}};$('identity').innerHTML=`<b>${esc(S.profile.full_name||S.profile.email||'Usuário')}</b>${esc(S.profile.role==='super_admin'?'Super Admin':S.profile.role==='teacher'?'Professor':'Aluno')}`}
