...
<section class="ai-section"><h4>Desvios</h4>${c1.map(d=>`<p><b>Trecho:</b> ${esc(d.original)}<br><b>Tipo:</b> ${esc(reportPortuguese(d.category))}<br><b>Explicação:</b> ${esc(d.rule)}<br><b>Sugestão:</b> ${esc(d.correction)}</p>`).join('')||'<p>Nenhum desvio confirmado identificado.</p>'}</section>
<section class="ai-section"><h4>Próximo passo</h4><p>${esc(r.next_step||r.overall_feedback||'Não informado')}</p></section>
${r.review_requirements?.length?`<div class="safe-note">${r.review_requirements.map(esc).join('<br>')}</div>`:''}
<details class="ai-section"><summary>Conferir evidências da avaliação</summary><p style="white-space:pre-wrap">${esc(r.transcription||'')}</p>${codes.map(c=>`<p><b>${c}:</b> ${esc(compData[c].diagnostic)}</p>`).join('')}<p>${esc(r.syntax_assessment||'')}</p><p>${esc(r.reading_notes||'')}</p>${(r.repertoire_checks||[]).map(x=>`<p>${esc(x.reference)}: ${esc(x.classification)}. ${esc(x.analysis)} ${/^https:\/\//.test(x.source_url||'')?`<a href="${esc(x.source_url)}" target="_blank" rel="noopener noreferrer">Fonte</a>`:''}</p>`).join('')}</details>`:`    <div class="score-total" data-ai-total>${total??'—'} / 1000</div>
...
`}
    ${jobStatus==='completed'?`<div class="form-section"><details data-ai-edit><summary>Editar correção por IA</summary><p>Revise as notas e os textos antes de publicar. A edição não inicia outra correção.</p><div class="comp-grid">${codes.map(c=>`<label class="field"><small>${c} · ${CC.find(x=>x[0]===c)[1]}</small><select data-ai-score="${c}">${CV.map(v=>`<option value="${v}" ${v===compData[c].score?'selected':''}>${v}</option>`).join('')}</select><textarea data-ai-just="${c}" aria-label="Justificativa ${c}">${esc(compData[c].diagnostic)}</textarea><small>Próximo passo</small><textarea data-ai-next="${c}">${esc(compData[c].improvement)}</textarea></label>`).join('')}</div><label class="field"><small>Ponto forte principal</small><textarea data-ai-strength>${esc(r.main_strength||'')}</textarea></label><label class="field"><small>Próximo passo / devolutiva ao aluno</small><textarea data-ai-feedback>${esc(r.overall_feedback||'')}</textarea></label><label class="field"><small>Anotações complementares do professor</small><textarea data-ai-deviations>${esc(r.detailed_analysis?.teacher_notes||'')}</textarea></label></details><label>Prioridade de melhoria<textarea data-ai-priority style="width:100%;min-height:80px">${esc(r.improvement_priority||'')}</textarea></label><p>Revise o resultado acima. Ao aprovar, a nota e a devolutiva serão disponibilizadas ao aluno.</p>${r.quality_version==='enem-evidence-2026-09-20'?`<details class="form-section" data-evidence-review><summary>Revisar correção</summary><h4>Conferir desvios no manuscrito</h4><p>Confira a lista como um conjunto. Os apontamentos permanecem confirmados por padrão; marque somente os que devem ser descartados.</p>${c1.map((d,i)=>`<div class="ai-section"><p><strong>${esc(d.original)}</strong> → ${esc(d.correction)}</p><p>${esc(d.rule)} · ${esc(d.location)}</p><blockquote>${esc(d.evidence)}</blockquote><label class="field"><small>Forma adequada</small><input data-deviation-correction="${i}" value="${esc(d.correction)}"></label><label class="field"><small>Regra aplicada no contexto</small><textarea data-deviation-rule="${i}">${esc(d.rule)}</textarea></label><label><input type="checkbox" data-deviation-discard="${i}"> Descartar este apontamento</label><label class="field"><small>Motivo do descarte (opcional)</small><input data-deviation-reason="${i}" placeholder="Leitura incorreta, variante aceita, sugestão de estilo..."></label></div>`).join('')||'<p>Nenhum desvio proposto. Confira a transcrição e o diagnóstico de C1.</p>'}<label class="field"><small>Observação da revisão (opcional)</small><textarea data-review-note placeholder="Registre algum ajuste ou observação, se necessário."></textarea></label><label><input type="checkbox" data-review-confirmed> Conferi a correção e os desvios acima. Os apontamentos não descartados estão confirmados.</label></details><button class="btn primary" data-approve-ai>Validar e publicar</button>`:'<div class="safe-note">Análise anterior ao controle de evidências. Use Correção manual para conferir e publicar sem nova cobrança, ou solicite uma nova análise.</div>'}<p data-approve-status role="status"></p></div>`:''}
  ...
  if(redo)redo.onclick=async()=>{
    ...
    try{if(box.isConnected)await aiCorrection(id,{redo:true,previousJob:job.id});}
  ...
  if(approve)approve.onclick=async()=>{
    ...
    const deviations=r.c1_deviations||[];
    edits.deviation_reviews=deviations.map((d,i)=>({index:i,decision:box.querySelector(`[data-deviation-discard="${i}"]`)?.checked?'discarded':'confirmed',reason:box.querySelector(`[data-deviation-reason="${i}"]`)?.value.trim()||'',correction:box.querySelector(`[data-deviation-correction="${i}"]`)?.value.trim()||'',rule:box.querySelector(`[data-deviation-rule="${i}"]`)?.value.trim()||''}));
    ...
    if(!box.querySelector('[data-review-confirmed]')?.checked)return required('[data-review-confirmed]','Confirme a revisão geral antes de publicar.');
    edits.review_confirmed=true;
    edits.detailed_analysis={...(edits.detailed_analysis||{}),c1_deviations:edits.deviation_reviews.filter(d=>d.decision==='confirmed').map(d=>({...deviations[d.index],correction:d.correction,rule:d.rule})),deviation_reviews:edits.deviation_reviews,teacher_review_note:edits.review_note};
...
async function aiShowJob(ctx,job){
  ...
  if(['completed','approved'].includes(job?.status)){
    aiPending(ctx.key,false);aiQueueStatus(ctx.id,job.status);
...
async function aiWatch(ctx,initialJob){
  ...
  while(aiPanelCurrent(ctx)&&Date.now()<deadline){
    ...
    if(!aiPanelCurrent(ctx))return;
    try{
      const current=await aiRead({action:'get',submission_id:ctx.id});job=current?.job||null;ctx.averageSeconds=current?.average_seconds||ctx.averageSeconds;ctx.history=current?.previous_job||ctx.history;
