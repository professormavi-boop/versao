...
`}
  ...
  if(approve)approve.onclick=async()=>{
    ...
    edits.deviation_reviews=deviations.map((d,i)=>({index:i,decision:box.querySelector(`[data-deviation-discard="${i}"]`)?.checked?'discarded':'confirmed',reason:'',correction:box.querySelector(`[data-deviation-correction="${i}"]`)?.value.trim()||'',rule:box.querySelector(`[data-deviation-rule="${i}"]`)?.value.trim()||''}));
