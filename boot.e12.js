...
(function(){
  ...
  let attempt=0, timer=null, ready=false, scriptsFailed=false;
  function available(){
  ...
  async function enter(event){
    ...
    try{
      const session=await signIn(element('email').value.trim(),element('password').value,true);
  ...
  async function initialize(){
    ...
    element('logoutBtn').onclick=()=>{ready=false;fail('');element('password').value='';};
