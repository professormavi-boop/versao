...
async function renderTeacherHome(navigation){
 ...
 if(d?.failed)pending.push(add(`${d.failed} correç${d.failed===1?'ão com falha':'ões com falha'}`,'correction','failed'));
 ...
 const priority=d?.pending||d?.validation||d?.failed?'correction':d&&!d.has_organizations?'class':d?.issues?.some(x=>x.kind==='import')?'import':d?.drafts||d?.ready?'proposals':'proposal';
 const correctionIntent=d?.pending?'uncorrected':d?.validation?'validation':d?.failed?'failed':'';
 const quick=[['proposal','Nova proposta','proposals','create'],['correction','Corrigir redações','correction',correctionIntent],['import','Importar alunos','teacher-organization','import'],['class','Nova turma','teacher-organization','class'],['student','Novo aluno','teacher-organization','student']];
