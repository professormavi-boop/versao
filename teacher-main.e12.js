...
const selectLiveFile=async e=>{
 ...
 try{const warnings=await inspectEssayPhoto(f);if(sequence!==photoSequence||!navigationCurrent(navigation))return;
  $('lvFeedback').textContent=warnings.length?warnings.join(' '):'Verificação inicial concluída. Amplie a prévia e confira a escrita; a análise automática não garante legibilidade.';
  if(warnings.length)return;selectedLiveFile=f;$('lvChecked').disabled=false;
 ...
 const controls=['lvOrg','lvClass','lvStudent','lvRound','lvCameraBtn','lvGalleryBtn','lvChecked'].map($);
 ...
 finally{controls.forEach(x=>x.disabled=false);button.disabled=!selectedLiveFile||!$('lvChecked').checked;button.textContent='Enviar para a fila'}
}}
